...
**倒排索引**，是通过**分词策略**，形成了词和文章的**映射关系表**，也称**倒排表**，这种<u>词典 + 映射表即为**倒排索引**</u>。
...
- 倒排索引往往字段很长，使用B+树的话，树可能会很深，造成IO变多
- 索引可能会失效，引起重建索引的问题
- 精准度低，召回率低
...
集群脑裂，是指 Elasticsearch 集群中的节点（比如共 20 个），其中的 10 个选了一个 master，另外 10 个选了另一个 master 的情况。
...
![elasticsearch_index_process.jpg](ElasticStack.assets/elasticsearch_index_process.jpg)
...
![elasticsearch_query_process.jpg](ElasticStack.assets/elasticsearch_query_process.jpg)
