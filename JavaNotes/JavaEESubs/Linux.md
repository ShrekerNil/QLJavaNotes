...
需要注意的是，不同的编程语言或项目可能会有特定的命名约定，因此最好遵循相应的规范。
...
| 根目录 | 作用 | 备注 |
| ------ | ---- | ---- |
|/bin | 基本工具箱 | 存放常用命令，如ls、cp 等 |
|/boot | 启动文件 | 包含内核和引导程序 |
|/dev | 设备接口 | 把硬件设备当作文件管理 |
|/etc | 系统配置 | 存放系统和软件的配置文件 |
|/home | 用户目录 | 每个用户的个人文件夹 |
|/lib | 系统库 | 程序运行所需的共享库文件 |
|/media | 可移动设备 | U盘、光盘的挂载点 |
|/sys | 硬件信息 | 提供内核和设备的实时数据 |
|/tmp | 临时文件 | 期存储，重启后自动清空 |
|/var | 日志与缓存 | 日志、缓存、邮件等动态数据 |
|/mnt | 临时挂载 | 手动挂载文件系统的地方 |
|/opt | 额外软件 | 存放用于安装额外的软件包 |
|/proc | 系统信息 | 存放运行时的内核和进程信息 |
|/root | 管理员目录 | 超级用户(root)的专属目录 |
|/run | 运行数据 | 存放临时数据，重启后清空 |
|/sbin | 管理员命令 | 系统管理工具，如shutdown |
|/srv | 服务数据 | 存放服务器数据，如网页资源 |
|/usr | 软件资源 | 存放大部分程序和库文件 |
...
如果你使用 `bash` 或 `sh` 命令调用子脚本，子脚本会在一个子Shell中执行。在这种情况下，可以使用 `exit` 来终止子脚本的执行，并返回到父脚本。示例：
...
- **`exit [n]`** 和 **`return [n]`** 后面跟的状态码用于表示脚本或函数的执行结果。
...
if [[ $signal -ge 128 ]]; then
    ...
    echo "Process was terminated by signal $actual_signal"
fi
```

- 如果 `./some_command.sh` 被 `SIGTERM` 终止，`signal` 将会是 `143`，`actual_signal` 将会是 `15`，因此输出将是 `Process was terminated by signal 15`。

### 总结

减去 `128` 是为了从退出状态码中提取出具体的信号编号，因为退出状态码被设置为 `128 + 信号编号` 以表示进程是被信号终止的。这种机制在 Shell 脚本中用于区分正常退出和被信号终止的情况。
