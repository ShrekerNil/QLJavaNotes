...
function set_prompt_dollar() {
    ...
    if grep -q 'PS1=' /root/.bashrc; then
        ...
        echo 'PS1 is already defined, modify dollar sign ...'
        sed -i '/PS1=/c\PS1="[\\u@\\h \\W]\\$ "' /root/.bashrc
    else
        echo 'PS1 does not exist, add a new PS1 definition ...'
        echo 'export PS1="[\\u@\\h \\W]\\$ "' >> /root/.bashrc
    fi
}

echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
echo "> Config Prompt sign ...                         <"
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
set_prompt_dollar
func_line_break
...
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
echo "> Uploading hosts on none Master nodes ...       <"
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
for hostname in ${!dict_host_unmaster[@]} ; do
    hostip=${dict_host_unmaster[$hostname]}
