...
> -- 王树森 云原生Go
>
> https://cloud.fynote.com/share/d/IcInOR5h?pwd=g3an
...
![image.png](K8S-CloudGo.assets/90998906d1f040cabfa8a92501ab73dc.png)

K8s要不要学？

![image.png](K8S-CloudGo.assets/9e6a06241dee46728bde9184d40c8a66.jpg)
...
![image.png](K8S-CloudGo.assets/523c14c863204efcb757f2695ee49fe6.png)

### k8s 部署方式

目前Kubernetes的两类部署样式

![image.png](K8S-CloudGo.assets/970170ede5cf4fcd908b19fdb579711e.png)

### k8s 环境解析

网络环境解析

![image.png](K8S-CloudGo.assets/f1b76d12294e4b6f83227004c054b601.png)

部署结构解析

![image.png](K8S-CloudGo.assets/83eaf14444d1474e966bf3dd49eceec6.png)
...
| 序号 | 主机ip       | 主机名规划                            |
| ---- | ------------ | ------------------------------------- |
| 1    | 192.168.1.50 | k8s-master.napsoul.com k8s-master     |
| 2    | 192.168.1.51 | k8s-node1.napsoul.com k8s-node1       |
| 3    | 192.168.1.52 | k8s-node2.napsoul.com k8s-node2       |
| 4    | 192.168.1.60 | k8s-registry.napsoul.com k8s-registry |
...
示例：多主机复制公钥
for i in <master-ip> <node-1-ip> <node-2-ip> <node-3-ip> <registry-ip>  \
do \
ssh-copy-id root@$i \
done

测试秘钥
for i in <master-ip> <node-1-ip> <node-2-ip> <node-3-ip> <registry-ip>; do ssh root@$i "hostname"; done
...
exec /bin/bash
配置其他节点主机名，每个节点执行一次
ssh root@<host-ip> "hostnamectl set-hostname <host-name>"
测试主机名
for i in <master-ip> <node-1-ip> <node-2-ip> <node-3-ip> <registry-ip>; do ssh root@$1 "hostname"; done
```
...
把这个k8s.conf传输到其他的主从节点
for i in <master-ip> <node-1-ip> <node-2-ip> <node-3-ip>  \
do \
scp /etc/sysctl.d/k8s.conf root@$i:/etc/sysctl.d/k8s.conf \
ssh root@$i "modprobe br_netfilter; modprobe overlay; sysctl -p /etc/sysctl.d/k8s.conf"
done
```
...
for i in <master-ip> <node-1-ip> <node-2-ip> <node-3-ip>; do
	scp /data/softs/cri-dockerd/cri-dockerd root@$i:/usr/local/bin/
done
...
$ for i in <master-ip> <node-1-ip> <node-2-ip> <node-3-ip> \
do \
scp /etc/systemd/system/cri-dockerd.service root@$i:/etc/systemd/system/cri-dockerd.service; done \
done
...
$ systemctl is-acative cri-dockerd
```

定制配置：cri-dockerd.socket
```sh
...
$ for i in <master-ip> <node-1-ip> <node-2-ip> <node-3-ip> \
do \
scp /etc/systemd/system/cri-dockerd.socket root@$i:/etc/systemd/system/cri-dockerd.socket; done \
done
```
...
```sh
修改配置
[root@k8s-registry /data/server/harbor]# vim harbor.yml.tmpl
# 修改主机名
...
```sh
登录仓库
# docker login k8s-registry.napsoul.com -u sswang
...
定制镜像标签
docker tag busybox k8s-registry.napsoul.com/sswang/busybox:v0.1

推送镜像
docker push k8s-registry.napsoul.com/sswang/busybox:v0.1
```
...
do
  ...
  docker tag registry.aliyuncs.com/google_containers/$i k8s-registry.napsoul.com/google_containers/$i
  docker push k8s-registry.napsoul.com/google_containers/$i
...
--image-repository k8s-registry.superopsmsb.com/google_containers \
...
do
    ...
    docker tag flannel/$i k8s-registry.superopsmsb.com/google_containers/$i
    docker push k8s-registry.superopsmsb.com/google_containers/$i
...
```sh
$ systemctl is-active kubectl cri-dockerd docker

# 配置开机自启
$ systemctl enable kubectl cri-dockerd docker
```
...
![image.png](K8S-CloudGo.assets/176c040ad69e4c6c84763b0058c62740.png)

### 应用部署实践

资源对象管理关系

![image.png](K8S-CloudGo.assets/4d41b8459755492e95944390172cd6fc.png)
...
spec:
  containers:
  - image: k8s-registry.napsoul.com/sswang/nginx
    name: my-pod
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: nginx
        image: k8s-registry.napsoul.com/sswang/nginx_web:v0.1
        ports:
        - containerPort: 80
tomcat-web 的资源清单文件
apiVersion: apps/v1
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: tomcat
        image: k8s-registry.napsoul.com/sswang/tomcat_web:v0.1
        ports:
        - containerPort: 8080
```
...
![image.png](K8S-CloudGo.assets/0f12d9679f354e59bfe5f33dfadce8e9.png)
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: flask-bookinfo
          image: k8s-registry.napsoul.com/sswang/flask_bookinfo:2.3.2
...
![image.png](K8S-CloudGo.assets/3eb86f24f6224c6b918605c3a12db22f.png)

k8s如何使用数据功能

![image.png](K8S-CloudGo.assets/07c9ecff65fe4a939a401ab178671974.png)

k8s使用各种数据类型的配置

![image.png](K8S-CloudGo.assets/b8196355709a402aa3522ea3db5840da.png)
...
spec:
  ...
  - name: change-index
    image: k8s-registry.napsoul.com/sswang/busybox:1.28
    # 每过2秒更改一下文件内容
...
spec:
  ...
  containers:
    - name: hostpath-redis
      image: k8s-registry.napsoul.com/sswang/redis:7.0.4
      volumeMounts:
       - name: redis-backup
         mountPath: /data
```
...
![image.png](K8S-CloudGo.assets/7132b215a8974a0db11bd3b689a9bee6.png)
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: nginx
        image: k8s-registry.napsoul.com/sswang/nginx_proxy:v0.1
        volumeMounts:
        - name: nginxconf
...
做成自签证书
openssl req  -new -x509 -key tls-key/tls.key  -out tls-key/tls.crt -subj "/CN=www.napsoul.com"
...
![image.png](K8S-CloudGo.assets/f88dcb5bf765417ca8675796f14cef7b.png)
...
do
  ...
  docker tag registry.cn-hangzhou.aliyuncs.com/google_containers/$i k8s-registry.napsoul.com/google_containers/$i
  docker push k8s-registry.napsoul.com/google_containers/$i
...
修改基础镜像
          ...
          image: k8s-registry.napsoul.com/google_containers/nginx-ingress-controller:v1.3.1
          image: k8s-registry.napsoul.com/google_containers/kube-webhook-certgen:v1.3.0
          image: k8s-registry.napsoul.com/google_containers/kube-webhook-certgen:v1.3.0
...
![image.png](K8S-CloudGo.assets/634d4622159f447e9e3612a44f266b14.png)
...
![image.png](K8S-CloudGo.assets/6efef8c8d5c64c7a853fd41507e67a9e.png)
